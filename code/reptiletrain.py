...
# Description: Meta-training script for Reptile-PINN. This code performs 
#              meta-training using a Reptile-like framework for PDE-based tasks.
...
def main(args):
    ...
    for step in range(args.epoch):
        ...
        rand_task_spt = random.sample(range(data_num), args.ntask)

        # 初始化参数差值累加字典
        diff = OrderedDict((name, torch.zeros_like(param)) for name, param in maml.named_parameters())
        ...
        for i in range(args.ntask):
            ...
            for name, param in maml.named_parameters():
                diff[name] += (params[name] - param)

        # 对所有任务更新求平均，并更新元模型参数
        for name, param in maml.named_parameters():
            param.data.add_(args.meta_lr * diff[name] / args.ntask)
