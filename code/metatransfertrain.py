...
def main(args):
    ...
    for step in range(args.epoch):
        ...
        rand_task_select = random.sample(range(data_num), args.ntask + 1)
        rand_task_spt = rand_task_select[0]   # support tasks
        rand_task_qry = rand_task_select[1:]   # query tasks

        # Support set data for the i-th task
        x = x_train[:, rand_task_spt].unsqueeze(1)
        z = z_train[:, rand_task_spt].unsqueeze(1)
        sx = sx_train[:, rand_task_spt].unsqueeze(1)
        m0 = m0_train[:, rand_task_spt].unsqueeze(1)
        m = m_train[:, rand_task_spt].unsqueeze(1)
        u0_real = u0_real_train[:, rand_task_spt].unsqueeze(1)
        u0_imag = u0_imag_train[:, rand_task_spt].unsqueeze(1)
